...
namespace EtabsAPI.SlabServices
{
    ...
    public class SlabHelper
    {
        ...
        public void CheckAllSlabsLoaded()
        {
            int NumberItems = 0;
	        string[] AreaName = {};
	        string[] LoadPat = {};
            ...
            foreach(var name in allSlabObj)
            {
                ...
                {
                    // This should be handled by consoleServices
                    Console.WriteLine($"WARNING: Area {name} : Not all load patterns are assigned please check the model");
                }
            }     
            Console.WriteLine($"CheckAllSlabsLoaded() Executed.");
